...
*build_nexus_blob.sh* script will run the Nexus container and configure it a docker repository. Then it will push all docker images from previously generated list to it. After all is done the repository container is stopped and its filesystem gets saved to resources directory.
