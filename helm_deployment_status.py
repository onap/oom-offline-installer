...
def get_ready(data):
    return [x for x in data if k8s_controller_ready(x)]
...
def analyze_k8s_controllers(resources_data):
    resources = {'ready_count': len(get_ready(resources_data))}
...
def parse_args():
    ...
    parser.add_argument('--k8s-url', '-u', help='address of Kubernetes cluster')
