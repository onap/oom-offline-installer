...
Before you dive into the installation you should prepare the offline installer itself - the installer consists of at least two packages/resources. You can read about it in the `Build Guide`_, where are the instructions how to create these.
...
That means the full deployment footprint is about ``224 GB RAM`` and ``112 vCPUs``. We will not follow strictly this setup, but we will deploy our installation across four nodes (VMs) instead of fourteen. You are free to diverge from our setup - you can follow the official guidelines or make completely different layout, but the minimal count of nodes should not drop below three - otherwise you may have to do some tweaking to make it work, which is not covered here.
...
You don't need to care about these services now - that is the responsibility of the installer. Just start four VMs as seen in this table (or according to your needs as we hinted above):
...
Unfortunately, the offline installer supports only **RHEL 7** distribution as of now. So, your VMs should be preinstalled with this operating system - the hypervisor and platform can be of your choosing.
...
- onap-installer-1.0-sw.tar
- onap-installer-1.0-resources.tar

We will store them in the ``/data`` directory on the *install-server* and then we will unpack the ``'sw'`` package to your home directory for example - that is the actual *offline-installer*::

    $ mkdir ~/onap-offline-installer
    $ tar -C ~/onap-offline-installer -xf /data/onap-installer-1.0-sw.tar
...
**NOTE:** The following paragraph describes a way how to create your own ``'application_configuration.yml'`` almost from scratch - we are discouraging you from executing this step. The recommended way is to use the packaged files inside the ``'application'`` directory.
...
**NOT RECOMMENDED:** If for some reason you don't have these files inside the ``'application'`` directory or you simply want to do things the hard way then you can recreate them from their templates. It is better to keep the originals (templates) intact - so we will copy them to the ``'application'`` directory::
    ...
    $ cp group_vars/all.yml application/application_configuration.yml
...
- Under the ``'resource-host'`` set the ``'ansible_host'`` address to the ip of your *install-server* - in our scenario that is ``'10.8.8.4'``.
...
In the ``'application_configuration.yml'`` file we will be interested in the following variables:
...
Both ``'resource_dir'`` and ``'resources_filename'`` applies to *install-server*. The ``'resource_dir'`` should be set to ``'/data'`` and ``'resources_filename'`` to ``'onap-installer-1.0-resources.tar'``. The values should be the same as are in the `Installer packages`_ section.

``'app_data_path'`` is the absolute path on the *infra-node* to where the packages will be extracted. The path is fully arbitrary, but it should point to the filesystem with enough space - the storage requirement in `Overview table of the kubernetes cluster`_.

The last variable ``'app_name'`` should be short and descriptive. We will set simply to: ``onap``.

It can look all together something like this::

    resources_dir: /data
    resources_filename: onap-installer-1.0-resources.tar
...
We are almost finished with the configuration and we are close to start the installation, but we need to setup passwordless login from *install-server* to the nodes.
...
If you built your ``'sw'`` package well then inside the ``'docker'`` directory should be the file ``'ansible_chroot.tgz'``. If not then you must create it - to learn how to do that and to get more info about the scripts dealing with docker and chroot, go to `Appendix 1. Ansible execution/bootstrap`_
...
Inside the ``'docker'`` directory is the ``'Dockerfile'`` and ``'build_ansible_image.sh'`` script. You can run ``'build_ansible_image.sh'`` script somewhere with the internet connectivity and it will download all required packages needed for building the ansible docker image and for exporting it into flat chroot environment.
...
The main purpose of this wrapper script is to provide the ansible framework to a machine where it was bootstrapped without need of installing additional packages. To display ``'ansible-playbook'`` command help one can run this::
...
To trigger this functionality and run ``'ansible-playbook'`` inside a docker container instead of the chroot environment, you must first set the ``ANSIBLE_DOCKER_IMAGE`` variable. The value must be a name of the built ansible docker image.
