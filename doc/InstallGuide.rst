...
- Under the ``'resource-host'`` set the ``'ansible_host'`` address to the ip of your server where the packages are stored - it must be reachable by ssh from the *infra-server*. In our scenario the *resource-host* is the same as the *install-server*: ``'10.8.8.4'``
