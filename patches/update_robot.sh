...
ROBOT_POD=`kubectl -n onap get pods | grep robot | awk {'print $1'}`
ROBOT_HOME="/var/opt/ONAP"

# distribute example private key onap-dev
kubectl -n onap cp onap-dev.pem ${ROBOT_POD}:${ROBOT_HOME}/onap-dev.pem
...
kubectl -n onap cp base_vfw.yaml ${ROBOT_POD}:${ROBOT_HOME}/demo/heat/vFW/base_vfw.yaml
