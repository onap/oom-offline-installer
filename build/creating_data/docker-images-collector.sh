...
usage () {
    echo "      "
    echo "  This script is preparing docker images list based on oom/onap project"
    echo "      Usage:"
    echo "        ./$(basename $0) <project version> <path to oom directory> [<output list file>]"
    echo "      "
    echo "      Example: ./$(basename $0) onap_3.0.1 /root/oom /root/docker_img.list"
    echo "      "
...
if [ "${1}" == "-h" ] || [ "${1}" == "--help" ] || [ $# -lt 2 ]; then
    usage
elif [ ! -d "${OOM_DIR}/kubernetes/onap" ]; then
    echo "Wrong path to oom project directory entered"
    exit 1
elif [ -z "${LIST}" ]; then
    mkdir -p ${LISTS_DIR}
...
# Make all
pushd "${OOM_DIR}/kubernetes"
make all; make onap
popd

# Create the list from all enabled subsystems
for subsystem in `parse_yaml "${OOM_DIR}/kubernetes/onap/values.yaml"`; do
    helm template "${OOM_DIR}/kubernetes/${subsystem}" | grep 'image:\ \|tag_version:\ \|h._image' |
