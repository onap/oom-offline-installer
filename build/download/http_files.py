...
def download(data_list, output_dir, progress):
    file_set = load_list(data_list)

    if progress is not None:
        progress.max_value = len(file_set)
...
def init_progress():
    progress_widgets = ['Downloading http files: ',
                        progressbar.Bar(), ' ',
...
def main():
    ...
    download(args.file_list, args.output_dir, progress)
