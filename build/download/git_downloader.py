...
class GitDownloader(command_downloader.CommandDownloader):
    ...
    @staticmethod
    def _clone_bare_repo(dst, repo, branch):
        """
        Clone single branch of bare repository
        :param dst: where to store repository
    ...
    def download(self):
        """
        Clone git repositories in lists
        """
        if not self._initial_log():
            return
        items_left = len(self._missing)
        error_occurred = False
        for repo, dst_dir in self._data_list.items():
            repo, branch = repo.split()
            dst = '{}/{}'.format(dst_dir, repo)
            try:
                self._clone_bare_repo(dst, repo, branch)
