...
def image_pulled(docker_client, image, pulled_images):
    if image_registry_name(image) in pulled_images:
        try:
            if verify_digest(docker_client, image):
                return True, image
            return False, image
...
def not_pulled_images(docker_client, target_list):
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=6) as executor:
        futures = [executor.submit(image_pulled, docker_client, image, pulled)
                   for image in target_list]
        return {future.result()[1] for future in concurrent.futures.as_completed(futures)
