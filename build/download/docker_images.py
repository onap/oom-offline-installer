...
def load_list(image_list):
    """
    Parse list with images to be downloaded.
    :param image_list: File with list of images (1 line per image)
    :return: set of images from file
    """
    with open(image_list, 'r') as f:
        return {image for image in (line.strip() for line in f) if image}
...
def download(image_list, save, output_dir, check_mode, progress):
    """
    ...
    :param progress_bar: enlighten.Counter progress bar to show how far download is
    :return: None
    """
...
def init_progress():
    progress_widgets = ['Downloading Docker images: ',
                        progressbar.Bar(), ' ',
...
def run():
    ...
    progress = init_progress() if not args.check else None
    sys.exit(download(args.image_list, args.save, args.output_dir, args.check, progress))
