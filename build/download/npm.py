...
def get_npm(registry, npm_name, npm_version):
    ...
    npm_req = requests.get(npm_url)
    npm_req.raise_for_status()
    npm_json = npm_req.json()
    tarball_url = npm_json['dist']['tarball']
    shasum = npm_json['dist']['shasum']
    tarball_req = requests.get(tarball_url)
    tarball_req.raise_for_status()
...
def download(npm_list, registry, dst_dir, check_mode, progress=None, workers=None):
    ...
    if progress is not None:
        ...
        progress.value = target_count - len(missing_npms)
...
def run_cli():
    ...
    progress = base.init_progress('npm packages', True) if not args.check else None
