...
## Required dependencies installed: nodejs, jq, docker
...
NXS_PYPI_LIST="${LISTS_DIR}/${TAG}-pip_packages.list"
...
for REGISTRY in `sed -e 's/\/.*$//' ${NXS_DOCKER_IMG_LIST} | sed -n '/\./p' | sort -u`; do
    if [[ ${REGISTRY} != *":"* ]]; then
...
SIMUL_HOSTS="$(sed -e 's/\/.*$// ; s/:.*$//' ${NXS_DOCKER_IMG_LIST} | sed -n '/\./p' | sort -u) ${NEXUS_DOMAIN}"
...
cp /etc/hosts "/etc/$(eval ${TIMESTAMP})_hosts.bk"
...
if [ -f ~/.docker/config.json ]; then
    mv ~/.docker/config.json ~/.docker/$(eval ${TIMESTAMP})_config.json.bk
fi
...
for REGISTRY in $(sed -e 's/\/.*$//' ${NXS_DOCKER_IMG_LIST} | sed -n '/\./p' | sort -u) ${DOCKER_REGISTRY}; do
    echo "Docker login to ${REGISTRY}"
    docker login -u "${NEXUS_USERNAME}" -p "${NEXUS_PASSWORD}" ${REGISTRY} > /dev/null
done

# Push docker images
for IMAGE in $(sed -n '/\..[^/]*\//p' ${NXS_DOCKER_IMG_LIST} | sed $'s/\r//' | awk '{ print $1 }'); do
    docker push ${IMAGE} > /dev/null
    echo "Image ${IMAGE} pushed to Nexus"
done
...
for IMAGE in $(sed -n '/\..[^/]*\//!p' ${NXS_DOCKER_IMG_LIST} | sed $'s/\r//' | awk '{ print $1 }'); do
    docker tag ${IMAGE} ${DOCKER_REGISTRY}/${IMAGE} > /dev/null
    docker push ${DOCKER_REGISTRY}/${IMAGE} > /dev/null
    echo "Image ${IMAGE} pushed to Nexus"
done
...
if [ -f ~/.docker/*_config.json.bk ]; then
    m -f "$(ls -tr ~/.docker/*_config.json.bk | tail -1)" ~/.docker/config.json
fi
