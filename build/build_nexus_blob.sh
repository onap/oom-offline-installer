...
NEXUS_IMAGE="${DATA_DIR}/offline_data/docker_images_infra/sonatype_nexus3_3.15.2.tar"
...
# Start the Nexus
NEXUS_CONT_ID=$(docker run -d --rm -v ${NEXUS_DATA_DIR}:/nexus-data:rw --name ${NEXUS_DOMAIN} -p 8081:8081 -p 8082:8082 -p 80:8082 -p 10001:8082 sonatype/nexus3)
echo "Waiting for Nexus to fully start"
until curl -qsu admin:admin123 http://${NEXUS_DOMAIN}:8081/service/metrics/healthcheck | grep '"healthy":true' > /dev/null ; do
    printf "."
    sleep 3
done
echo -e "\nNexus started"
...
curl -sX POST --header 'Content-Type: application/json' --data-binary "${NEXUS_CONFIG}" http://admin:admin123@${NEXUS_DOMAIN}:8081/service/rest/v1/script
curl -sX POST --header "Content-Type: text/plain" http://admin:admin123@${NEXUS_DOMAIN}:8081/service/rest/v1/script/configure/run > /dev/null
...
for REGISTRY in $(grep '.*\..*/' ${NXS_DOCKER_IMG_LIST} | sed 's/\/.*//g' | sort -u) ${NEXUS_DOMAIN}:8082; do
    echo ${REGISTRY}
