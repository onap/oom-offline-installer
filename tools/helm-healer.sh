...
DESCRIPTION
    ...
    redeploy onap component. It can fix only problems related
    to race condition or timeout. Nothing else. It will not fix
    broken ONAP.
...
USAGE
    ...
    mandatory for this script to execute

    CAUTION: filename of an override file cannot contain whitespace!
    This is actually helm issue which does not handle such files.
    So I dropped the more complicated version of this script when
    there is no reason to support something on what will helm choke
    anyway.
...
delete_jobs()
{
    msg "Delete jobs for ${1}..."
    kubectl get jobs -n ${NAMESPACE} \
        --ignore-not-found=true \
        --selector="release=${1}" \
        --no-headers=true | \
        while read -r _job _rest ; do
            delete_job "$_job"
        done
}
...
redeploy_component()
{
    ...
    #for x in deployment pod pvc pv secret clusterrolebinding ; do
    for x in deployment pod pvc pv ; do
        delete_resource ${x} ${1}
    done
