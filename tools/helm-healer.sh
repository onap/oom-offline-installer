...
{
    _resource="$1"
    _kind="$(echo ${_resource}| cut -d "/" -f 1)"
    _name="$(echo ${_resource}| cut -d "/" -f 2)"
...
delete_persistant_volume()
{
    _persistant_volume=$1
     if kubectl get ${_persistant_volume} >/dev/null 2>&1; then
          msg "${_persistant_volume} has not been removed with helm undeploy, manual removal is required. Proceeding"
          #very often k8s hangs on Terminating state for pv due to  still active pvc. It is better to delete pvc directly
         _claim=$(kubectl get ${_persistant_volume} -o jsonpath='{ .spec.claimRef.name}')
...
undeploy_component()
{
    ...
    for resource in ${_component_resources}; do
        case $resource in
            Secret/* | ConfigMap/* | Pod/* | Service/* | Deployment/* | StatefulSet/*)
                _standard="${_standard} ${resource}";;
            PersistentVolume/*)
                _persistent_volumes="${_persistent_volumes} ${resource}";;
            *)
                _unknown_kinds="${_unknown_kinds} ${resource}"
        esac
    done
    ...
    for volume in ${_persistent_volumes}; do
        _physical_locations="${_physical_locations} $(kubectl get ${volume} -o jsonpath='{ .spec.hostPath.path}' )"
    done
    ...
    for volume in ${_persistent_volumes}; do
        delete_persistant_volume ${volume}
    done
    ...
    if [ -z "_unknown_kinds" ] ; then
        error "Untracked resource kind present: ${_unknown_kinds}"
        return
    fi
}
