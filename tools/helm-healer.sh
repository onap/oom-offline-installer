...
USAGE
    ...
    which will delete all components - healthy or not.
EOF
}
...
EOF
            ...
            msg "Delete directories '${VOLUME_STORAGE}/${RELEASE_PREFIX}-*' on $_node"
            ssh -T $_node <<EOF
rm -rf "${VOLUME_STORAGE}/${RELEASE_PREFIX}-"*
EOF
        fi
    fi
}
...
if [ -n "$arg_deleteall" ] ; then
    HELM_DELETE_ALL_RELEASES=DELETEALL
fi
...
# if --delete-all is used then redeploy all components (found by release label)
elif [ -n "$HELM_DELETE_ALL_RELEASES" ] ; then
    msg "Delete ALL components..."
    _COMPONENTS=$(get_labels)
...
# sometimes no pod of a release is currently running but it could left some files behind...
if [ -n "$HELM_DELETE_ALL_RELEASES" ] && [ -n "$VOLUME_STORAGE" ] ; then
    msg "Delete all other stale volume directories we missed..."
    delete_storage
fi
