...
def create_blacklist(config_file):
    """
    Generate a list of local images which needs to be excluded from upstream docker_image_list
    :param config_file: application_configuration file where local images are.
    :return:
    """
...
def should_remove_line(line, blacklist):
    """
    Helping function to match image image in blacklist
...
def run_cli():
    ...
    parser = ModifiedParser(description='Remove local runtime images from docker_image_list')

    parser.add_argument('config_file',
                        help='application_configuration file where local images are')
